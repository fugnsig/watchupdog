...
_cfg = load_config()
app = FastAPI(title="watchupdog dashboard", version="0.1.0")
...
<body>
<h1>watchupdog &nbsp;<span id="status-badge">…</span></h1>
<div id="alerts"></div>
...
def serve(host: str = "127.0.0.1", port: int = 8190, reload: bool = False) -> None:
    uvicorn.run("watchupdog.dashboard_server:app", host=host, port=port, reload=reload)
