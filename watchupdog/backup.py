...
_BACKUP_DIR_USER = Path.home() / ".watchupdog" / "backups"
...
def _safe_req_lines(lines: list[str]) -> list[str]:
    ...
    for raw_line in lines:
        # Split on embedded newlines before checking each segment.
        # A single JSON string like "legit==1.0\n-i https://evil.com" would
        # pass the leading-dash check on the whole string (starts with 'l')
        # but become two physical lines in the requirements file, letting pip
        # treat the second segment as an option flag.  Splitting first ensures
        # every physical line pip will see gets the same leading-dash filter.
        for segment in raw_line.splitlines():
            line = segment.strip()
...
def _backup_dir() -> Path:
    ...
    Fallback: ~/.watchupdog/backups/ — used when the primary location is
...
def _snapshot_path(ts: str, install_name: str = "") -> Path:
    """
    pip_state_ComfyUI_20250325_143022.json   ← with install folder name
    pip_state_20250325_143022.json           ← fallback when name unavailable
    """
...
def restore_snapshot(
    ...
    if comfyui_root:
        if snap_root_str:
            ...
            if snap_root_resolved != comfyui_root:
                snap_root_exists = (
                    snap_root_resolved is not None
                    and snap_root_resolved.exists()
                )
                if snap_root_exists:
                    # Path exists locally → genuinely wrong installation on
                    # this machine.  Hard-block to prevent cross-install clobber.
                    ...
                    # Path does not exist on this machine — cross-machine
                    # transfer scenario (different drive letter, different PC).
                    # Cannot verify installation identity; warn and continue.
                    messages.append(
                        f"[WARN] Snapshot path does not exist on this machine "
                        f"({snap_root_str}) — cross-machine restore assumed. "
                        f"Packages will be installed into: {comfyui_root}"
                    )
            else:
                messages.append(f"[OK] Installation path verified: {comfyui_root}")
                # ── Git hash cross-check ───────────────────────────────────
                # Two installs at the same path (e.g. delete + reinstall) are
                # indistinguishable by path alone.  Comparing the ComfyUI git
                # hash catches the common case where the reinstall cloned a
                # different commit, and is also useful context in normal use.
                snap_git = snap_comfyui.get("git_hash", "")
                if snap_git and comfyui_root:
                    rc_g, cur_git, _ = _run(
                        ["git", "rev-parse", "HEAD"],
                        cwd=str(comfyui_root),
                    )
                    cur_git = cur_git.strip()
                    if rc_g == 0 and cur_git and cur_git != snap_git:
                        messages.append(
                            f"[WARN] ComfyUI commit has changed since this snapshot was taken.\n"
                            f"  Snapshot : {snap_git[:12]}\n"
                            f"  Current  : {cur_git[:12]}\n"
                            "  Packages may not match the current ComfyUI version."
                        )
        else:
            messages.append(
    ...
    if snap_py_ver and exe:
        if not Path(exe).exists():
            # If python_exe was not passed explicitly the exe came from the
            # snapshot.  That path is valid on the source machine but won't
            # exist here — guide the user to supply the local path.
            hint = (
                "  Use --comfyui-path to point at the local ComfyUI installation\n"
                "  so the correct venv Python can be used for the restore."
                if python_exe is None
                else "  The ComfyUI environment may have been moved or deleted.\n"
        ...
        rc_py, cur_py_raw, cur_py_err = _run([exe, "--version"])
        cur_py_ver = cur_py_raw.strip()
        snap_mm = _parse_major_minor(snap_py_ver)
        cur_mm  = _parse_major_minor(cur_py_ver)

        # Warn when the snapshot records a version string but it can't be
        # parsed — version compatibility check will be silently skipped
        # without this diagnostic, masking a potentially corrupt snapshot.
        if snap_py_ver and snap_mm is None:
            messages.append(
                f"[WARN] Cannot parse Python version from snapshot "
                f"('{snap_py_ver}') — version compatibility check skipped."
            )

        # Hard-block if the executable exists but cannot run at all.
        # A corrupt or incomplete python_embeded passes Path.exists() but
        # fails here with rc != 0 and empty output — cur_mm will be None.
        if rc_py != 0 or not cur_py_ver:
            err_detail = (cur_py_err or "").strip()[:120]
            return False, messages + [
                "",
                "┌─ BLOCKED ──────────────────────────────────────────────────────┐",
                "│  Python executable exists but failed to run                     │",
                "└────────────────────────────────────────────────────────────────┘",
                f"  Executable : {exe}",
                *([ f"  Error      : {err_detail}"] if err_detail else []),
                "",
                "  The embedded Python may be corrupt, incomplete, or built for a",
                "  different architecture.  Reinstall ComfyUI to restore a working",
                "  Python environment before attempting a package restore.",
            ]
