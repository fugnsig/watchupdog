...
def _check_python_env() -> list[EnvCheckRow]:
    ...
    if ver >= (3, 10):
        ...
        rows.append(
            EnvCheckRow(
                ...
                f"Download 3.12 from https://python.org/downloads/ then relaunch.",
                fix_cmd=None,  # no command can install a newer Python automatically
            )
        )
    ...
    if _comfyui_root and _comfyui_root.is_dir():
        ...
        if _found_env:
            ...
            # Before warning, check whether a conda environment is active.
            # CONDA_PREFIX is set by `conda activate` and identifies the env
            # unambiguously — no need to shell out to conda.
            _conda_prefix = os.environ.get("CONDA_PREFIX", "").strip()
            if _conda_prefix:
                rows.append(EnvCheckRow(group, "Virtual environment", STATUS_OK,
                                        f"ComfyUI uses conda env: {_conda_prefix}"))
            else:
                rows.append(EnvCheckRow(group, "Virtual environment", STATUS_WARN,
                                        f"No .venv, embedded Python, or active conda env found "
                                        f"for {_comfyui_root.name} "
...
def _win_drives() -> list[str]:
    """Return drive-letter prefixes (e.g. ['C:', 'D:', 'E:']) for all currently
    mounted Windows drives, including external HDDs and SSDs.

    Falls back to a minimal set if psutil is unavailable.
    """
    try:
        import psutil
        drives: list[str] = []
        for part in psutil.disk_partitions(all=False):
            ...
            if mp and len(mp) == 2 and mp[1] == ":":
                drives.append(mp)
        return drives if drives else ["C:", "D:", "E:", "F:"]
    except Exception:
        return ["C:", "D:", "E:", "F:"]
...
def _get_comfyui_dirs() -> list[Path]:
    ...
    # 4. Windows-specific common locations — all mounted drives (incl. external)
    for drive in (_win_drives() if sys.platform == "win32" else []):
    ...
    if sys.platform == "win32":
        ...
        # Portable zip layout: some_folder/ComfyUI/main.py — all mounted drives
        for _drive in _win_drives():
    ...
    # Unix-like (Linux, FreeBSD, OpenBSD, etc.): external drives at /media and /mnt.
    # /media/<user> is the automount convention on modern desktop Linux;
    # /media and /mnt are used broadly across all Unix-like systems.
    # Guarded to exclude Windows (drives handled above) and macOS (/Volumes below).
    if sys.platform not in ("win32", "darwin"):
        for _mount_root in [Path("/media") / home.name, Path("/media"), Path("/mnt")]:
            if _mount_root.is_dir():
                try:
                    for _vol in sorted(_mount_root.iterdir()):
                        if not _vol.is_dir():
                            continue
                        add(_vol / "ComfyUI")
                        try:
                            for _d in sorted(_vol.iterdir()):
                                if _d.is_dir() and "comfyui" in _d.name.lower():
                                    add(_d)
                        except (PermissionError, OSError):
                            pass
    ...
    # macOS: external drives mounted under /Volumes
    if sys.platform == "darwin":
        _volumes = Path("/Volumes")
        if _volumes.is_dir():
            try:
                for _vol in sorted(_volumes.iterdir()):
                    if not _vol.is_dir():
                        continue
                    add(_vol / "ComfyUI")
                    try:
