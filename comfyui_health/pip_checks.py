...
def run_pip_checks(comfyui_root: Path | None = None) -> list[EnvCheckRow]:
    ...
    python_exe = venv_python_for_root(root)

    if python_exe is None:
        rows.append(EnvCheckRow(group, "ComfyUI venv Python", STATUS_FAIL,
                                f"No venv found inside {root}. Set COMFYUI_PATH to the correct directory."))
        return rows

    rows.append(EnvCheckRow(group, "ComfyUI venv Python", STATUS_OK, str(python_exe)))

    # 2. Probe the venv
    probe = probe_venv(python_exe)
    if probe is None:
        rows.append(EnvCheckRow(group, "Venv probe", STATUS_FAIL,
                                "Failed to run probe script in ComfyUI Python"))
        return rows

    rows.append(EnvCheckRow(group, "Python (venv)", STATUS_OK,
