"""Click entry point for comfyui-health CLI."""
...
def main(
    ...
    """ComfyUI Health Monitor — check the status of a running ComfyUI instance."""
    ...
    if diff_target is not None or (len(sys.argv) > 1 and "--diff" in sys.argv):
        ...
        if diff_target and diff_target not in ("latest", "previous", None):
            ...
            if _match is None:
                console.print(
                    f"[yellow]Warning:[/yellow] no snapshot matched [bold]{diff_target!r}[/bold] — "
                    "comparing the two most recent snapshots instead."
                )
                snap_a = snaps[1]
            else:
                snap_a = _match
    ...
    if do_backup:
        ...
        if do_checksums:
            console.print("[dim]Collecting snapshot with SHA-256 model checksums — this may take several minutes...[/dim]")
        else:
            console.print("[dim]Collecting snapshot — this may take a moment...[/dim]")
    ...
    if do_backup_workflows:
        ...
        if _wf_root is None:
            ...
            sys.exit(1)
        console.print("[dim]Archiving workflow files...[/dim]")
        try:
    ...
    if do_missing_models:
        ...
        console.print(f"[dim]Checking against snapshot: {snap_file}[/dim]\n")
        if _mm_root is None:
